#LUSC_GSE18842 91samples，###Affymetrix
library(affy)
library(limma)
library(biomaRt)
library(sva)
...
##选择工作路径
setwd("D:\\鳞癌分析\\泛鳞癌array数据\\LUSC\\GSE18842_RAW")
dir = getwd()
...
GSE18842_raw<- ReadAffy(filenames = cel.files) #读取文件
sampleNames(GSE18842_raw) #列出样本名
#the length of name maybe 8,9 or 10 #将样本名定义为前8，9,或10位
sampleNames(GSE18842_raw)<-stri_sub(sampleNames(GSE18842_raw),1,9)#the length of name maybe 8,9 or 10 #将样本名定义为前8位
