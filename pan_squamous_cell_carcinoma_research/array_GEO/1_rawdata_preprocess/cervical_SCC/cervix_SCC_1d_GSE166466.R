###cervix_SCC_1c_GSE166466 #[Clariom_D_Human] Affymetrix Human Clariom D Assay [transcript (gene) version]
library(stringi);library(pd.hugene.2.0.st);library(oligo)


setwd("D:\\鳞癌分析\\泛鳞癌array数据\\cervix_SCC\\GSE166466_RAW")
dir = getwd()
...
GSE166466_raw<- read.celfiles( cel.files ) #读取文件
sampleNames(GSE166466_raw) #列出样本名
#the length of name maybe 8,9 or 10 #将样本名定义为前8，9,或10位
sampleNames(GSE166466_raw)<-stri_sub(sampleNames(GSE166466_raw),1,10)#the length of name maybe 8,9 or 10 #将样本名定义为前8位
